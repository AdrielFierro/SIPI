...
const Grupo = () => {
    ...
    useEffect(() => {
        const fetchUsuarios = async () => {
            try {
                const response = await fetch(`https://sipi-back-7dkp.onrender.com/grupos/usuario/${nombre_grupo}`);
                if (!response.ok) {
    ...
    const obtenerRecomendaciones = async (movieId) => {
      ...
      const obtenerUltimaFavorita = async (usuario) => {
        try {
          const response = await fetch(`https://sipi-back-7dkp.onrender.com/user/ultimaFavorita/${usuario}`);
          const data = await response.json();
